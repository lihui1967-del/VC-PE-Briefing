...
SECTOR_RULES = {
    ...
    "硬科技": ["芯片", "半导体", "EDA", "晶圆", "封装", "传感", "光电", "光刻",
               ...
               "机床", "工业母机", "工业软件", "材料", "相变", "散热", "热管理",
               "冷却", "DPU", "GPU", "CPU", "功率器件", "MEMS"],
    "前沿科技": ["量子", "中性原子", "脑机", "BCI", "核聚变", "可控核聚变", "卫星",
                 "空间", "合成生物", "超导", "新材料", "碳纤维", "芳纶", "核能"],
...
# 两种合法形态：
#   (a) 程度词 + 可选数字 + 量级词 —— "超亿元" "近3亿" "逾千万"
#   (b) 数字/数量词 + 量级词      —— "5000万" "数千万" "1.2亿"
# 光一个"万/亿"不接受，否则"万科""万华"会被误判。
...
# 出现这些词，说明主语是"被投企业拿到钱"，而不是"基金本身有动作"。
# 用来挡住「XX完成D+轮融资，YY产业投资基金独家投资」这类误入基金池的条目。
PORTFOLIO_SIGNAL_WORDS = [
    "轮融资", "战略融资", "战略投资", "战略入股", "天使轮", "种子轮",
    "Pre-A", "Pre-B", "A轮", "B轮", "C轮", "D轮", "E轮", "F轮", "轮次",
    "领投", "跟投", "参投", "独家投资", "独投", "加持", "增资",
]
...
def is_fund_news(title: str) -> bool:
    if not title or is_noise(title):
        return False
    # 先排除被投企业新闻 —— 投资方叫什么基金不代表这是基金动态
    if has_any(title, PORTFOLIO_SIGNAL_WORDS):
        return False
